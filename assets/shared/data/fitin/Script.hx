...
function onUpdatePost(dt){
	...
	if(noError){
		...
		if(valid){
			...
			var newPos:Float = lerp(oldPos, oldPos + (1 / game.camGame.zoom * offset), dt * game.cameraSpeed * game.playbackRate * 1.8);
...
function lerp(start:Float, end:Float, speed:Float):Float{
	return start + (end - start) * speed * 3;
}