...
class DropShadowScreenspace extends DropShadowShader
{
  ...
  public var angle2(default, set):Float;
  public var doubleRim(default, set):Bool;
  ...
  function set_angle2(val:Float):Float {
    angle2 = val;
    ang2.value = [val * FlxAngle.TO_RAD];
    return val;
  }

  function set_doubleRim(val:Bool):Bool {
    doubleRim = val;
    douRim.value = [val];
    return val;
  }
  ...
  @:glFragmentSource('
      ...
      vec3 createDropShadow(vec3 col, float curThreshold, bool useMask) {
        ...
        if(douRim){
          checkedPixel2 = vec2(openfl_TextureCoordv.x + ((dist*zoom) * cos(ang2 + angOffset) * imageRatio.x), openfl_TextureCoordv.y - ((dist*zoom) * sin(ang2 + angOffset) * imageRatio.y));
          float dropShadowAmount2 = 0.0;
          dropShadowAmount2 = texture2D(bitmap, checkedPixel2).a;
          col.rgb += dropColor.rgb * ((1.0 - (dropShadowAmount2 * str))*intensity);
        }
