...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			// case 'custom': new states.stages.Highschool();//Custom Song
		...
		if (!stageData.hide_girlfriend)
		{
			...
			if(curStage == 'leafstorm') gf.scale.set(0.7,0.7);
		...
		if(curStage == 'tower') scoreTxt.setFormat(Paths.font("poke.otf"), 30, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							else if (daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote && !daNote.forceOppMiss)
							...
							if (Conductor.songPosition - daNote.strumTime > noteKillOffset)
							{
								if (daNote.mustPress && !cpuControlled && !daNote.ignoreNote && !endingSong && (daNote.tooLate || !daNote.wasGoodHit))
									noteMiss(daNote);
								else if(!daNote.mustPress && daNote.forceOppMiss)
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Cinema Time'://Cinematic Bars
				...
				bartop.screenCenter(X);
				insert(1, bartop);
				add(bartop);
				barbot = new BGSprite("bar", 0, 720, 0, 0);
				barbot.cameras = [camHUD];
				barbot.screenCenter(X);
				insert(1, barbot);
				add(barbot);

				FlxTween.tween(bartop, {y:0}, 1.5, {
					ease:FlxEase.cubeInOut, onComplete: function(twn:FlxTween){}});
				FlxTween.tween(barbot, {y:640}, 1.5, {
					ease:FlxEase.cubeInOut, onComplete: function(twn:FlxTween){}});
		}
	...
	function missingnoThing() {//"barrowed" from the old P.E. port of Hypno's Lullaby, sorry Yoshubs
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if(char != null)
			{
				char.playAnim(animToPlay, true);
				char.holdTimer = 0;
				if(curStage == '7quid') bias = Math.min(bias + 0.4, base);
	...
	function opponentMissChance(note:Note):Void
	{
		...
		if(missed && !note.forceOppMiss && !note.isSustainNote && distance >= 9.5 && distance <= 11.5){
			// Realized that Binej never missed in the middle of a sustain, just it's 1st note
			note.forceOppMiss = missed;
		}
	}
	...
	function opponentnoteMiss(direction:Int, note:Note = null)
	{
		...
		if (note != null && guitarHeroSustains && note.parent == null) {
			if(note.tail.length > 0) {
				...
				for(childNote in note.tail) {
					childNote.alpha = note.alpha;
					childNote.forceOppMiss = true;
				}
				note.forceOppMiss = true;
		...
		if (note != null && guitarHeroSustains && note.parent != null && note.isSustainNote) {
			...
			if (parentNote.wasGoodHit && parentNote.tail.length > 0) {
				for (child in parentNote.tail) if (child != note) {
					child.forceOppMiss = true;
				}
			}
		}

		health += (add * healthLoss);
		bias = Math.max(bias - 7.5, 0);
			...
			if(!note.noMissAnimation) {
				switch(note.noteType) {
					...
					case 'Flash Note':
						...
						flash = new BGSprite('camFlash',0,0);
						flash.scale.set(2,2);
						flash.cameras = [camOther];
						add(flash);
						FlxTween.tween(flash, {alpha: 0}, 0.3, {ease: FlxEase.quartIn, onComplete: 
					...
					case 'Sus Note':
						...
						// if(curStage == 'sus' || curStage == 'cupNM'){
						// 	if(daTimingStarted){
						// 		daTiming.cancel();
						// 		darkTween1 = null;
						// 		darkTween2 = null;
						...
						// 		darkTween4 = null;
						// 	}
						// 	if(accounting >= 1)susDark1.alpha = 1;
						// 	if(accounting >= 2)susDark2.alpha = 1;
						// 	if(accounting >= 3)susDark3.alpha = 1;
						// 	if(accounting >= 4 && curStage != 'cupNM')susDark4.alpha = 1;

						// 	daTimingStarted = true;
