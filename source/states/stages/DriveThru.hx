...
class DriveThru extends BaseStage
{
	...
	override function update(elapsed:Float)
	{
		// Code here
		if(!switched){
			for (i in 0...game.opponentStrums.length)newX[i] = game.opponentStrums.members[i].x;
			for (i in 0...game.opponentStrums.length)game.opponentStrums.members[i].x = game.playerStrums.members[i].x;
			for (i in 0...game.playerStrums.length)game.playerStrums.members[i].x = newX[i];
			switched = true;
		}
	}
